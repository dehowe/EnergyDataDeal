...
def S1_Combine_s(path):  #合并数据
    ...
    try:
        for i in range(18):  #提取s数据
            ...
            csvfile.close()
            data_csv.columns = ['date', 'v', 'a', 'lei', 'zai', 'mileage', 'stop_num','accelerate','GPS_1','GPS_2','GPS_3','GPS_4','GPS_5']

            data_csv = data_csv.drop(['mileage', 'stop_num', 'accelerate','GPS_1','GPS_2','GPS_3','GPS_4','GPS_5'], axis=1)
...
check_file_integrity(path_out) #检查数据完整性
